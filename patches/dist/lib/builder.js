...
import { createReadStream, createWriteStream, existsSync, rename, mkdir, readFile, writeFile, appendFile } from './utils/node-fs.js';
import { streamToString } from './utils/stream.js';
import { encodeInput, generatePrefetchCode } from './rsc/utils.js';
import { RSDW_SERVER_MODULE, RSDW_SERVER_MODULE_VALUE, renderRsc, getBuildConfig } from './rsc/rsc-renderer.js';
import { REACT_MODULE, REACT_MODULE_VALUE, RD_SERVER_MODULE, RD_SERVER_MODULE_VALUE, RSDW_CLIENT_MODULE, RSDW_CLIENT_MODULE_VALUE, WAKU_CLIENT_MODULE, WAKU_CLIENT_MODULE_VALUE, renderHtml } from './rsc/html-renderer.js';
...
const buildServerBundle = async (rootDir, config, entriesFile, distEntriesFile, commonEntryFiles, clientEntryFiles, serverEntryFiles)=>{
    const serverBuildOutput = await viteBuild({
        plugins: [
            nonjsResolvePlugin(),
            rscTransformPlugin(true, config.assetsDir, {
...
const emitRscFiles = async (rootDir, config, distEntriesFile)=>{
    ...
    await Promise.all(Object.entries(buildConfig).map(async ([, { entries, context }])=>{
        for (const [input] of entries || []){
            const destRscFile = joinPath(rootDir, config.distDir, config.publicDir, config.rscPath, encodeInput(// Should we do this here? Or waku/router or in entries.ts?
            input.split('\\').join('/')));
...
`;
    const htmlFiles = await Promise.all(Object.entries(buildConfig).map(async ([pathStr, { entries, customCode, context }])=>{
        const destHtmlFile = joinPath(rootDir, config.distDir, config.publicDir, extname(pathStr) ? pathStr : pathStr + '/' + config.indexHtml);
        const destHtmlJsFile = joinPath(rootDir, config.distDir, config.htmlsDir, (extname(pathStr) ? pathStr : pathStr + '/' + config.indexHtml) + '.js');
        loadHtmlCode += `    case ${JSON.stringify(pathStr)}:
      return import('./${joinPath(config.htmlsDir, (extname(pathStr) ? pathStr : pathStr + '/' + config.indexHtml) + '.js')}').then((m)=>m.default);
`;
        let htmlStr;
        if (existsSync(destHtmlFile)) {
            htmlStr = await readFile(destHtmlFile, {
                encoding: 'utf8'
            });
        } else {
        ...
        await mkdir(joinPath(destHtmlJsFile, '..'), {
            recursive: true
        });
        ...
        const htmlResult = ssr && await renderHtml({
            config,
            pathStr,
            htmlStr,
            context,
        ...
        if (htmlResult) {
            ...
            await Promise.all([
                pipeline(Readable.fromWeb(htmlReadable1), createWriteStream(destHtmlFile)),
                streamToString(htmlReadable2).then((str)=>writeFile(destHtmlJsFile, `export default ${JSON.stringify(str)};`))
            ]);
        } else {
            await Promise.all([
                writeFile(destHtmlFile, htmlStr),
                writeFile(destHtmlJsFile, `export default ${JSON.stringify(htmlStr)};`)
            ]);
        }
        return destHtmlFile;
    }));
...
export async function build(options) {
    ...
    const rootDir = (await viteResolveConfig({}, 'build', 'production', 'production')).root;
